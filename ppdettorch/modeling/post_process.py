...
from ppdettorch.modeling.bbox_utils import nonempty_bbox
from ppdettorch.modeling.layers import TTFBox
    ...
    def __call__(self, head_out, im_shape, scale_factor):
        ...
        if not self.use_focal_loss:
            ...
            if scores.shape[1] > self.num_top_queries:
                ...
                labels = torch.stack(
                    [torch.gather(l, i) for l, i in zip(labels, index)])
                bbox_pred = torch.stack(
                    [torch.gather(b, i) for b, i in zip(bbox_pred, index)])
        else:
            scores, index = torch.topk(
