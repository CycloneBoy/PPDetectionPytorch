...
from ppdettorch.modeling.bbox_utils import bbox_iou
...
class YOLOv5Loss(nn.Module):
    ...
    def build_targets(self, outputs, targets, anchors):
        ...
        for i in range(len(anchors)):
            ...
            indices.append((torch.FloatTensor(b), torch.FloatTensor(a),
                            torch.LongTensor(gj), torch.LongTensor(gi)))
            tbox.append(
                torch.FloatTensor(
                    np.concatenate((gxy - gij, gwh), 1), dtype=torch.float32))
            anch.append(torch.FloatTensor(anchor[a]))
            tcls.append(torch.FloatTensor(c))
    ...
    def yolov5_loss(self, pi, t_cls, t_box, t_indices, t_anchor, balance):
        ...
        loss_box = torch.FloatTensor([0.])
        loss_cls = torch.FloatTensor([0.])
        if n:
            ps = pi.gather_nd(
                torch.concat([
            ...
            pbox = torch.concat((pxy, pwh), 1)  # predicted box # [21, 4]
            iou = bbox_iou(pbox.T, t_box.T, x1y1x2y2=False, ciou=True)
            # iou.stop_gradient = True
            loss_box = (1.0 - iou).mean()

            # Objectness
            score_iou = torch.cast(iou.detach().clip(0), tobj.dtype)
            with torch.no_grad():
                tobj[b, a, gj, gi] = (1.0 - self.gr
                                      ) + self.gr * score_iou  # iou ratio

            # Classification
            t = torch.full_like(ps[:, 5:], self.cls_neg_label)
