...
class GIoULoss(object):
    ...
    def __call__(self, pbox, gbox, iou_weight=1., loc_reweight=None):
        x1, y1, x2, y2 = torch.split(pbox, num_or_sections=4, dim=-1)
        x1g, y1g, x2g, y2g = torch.split(gbox, num_or_sections=4, dim=-1)
        box1 = [x1, y1, x2, y2]
...
class DIouLoss(GIoULoss):
    ...
    def __call__(self, pbox, gbox, iou_weight=1.):
        ...
        intsctk = intsctk * torch.greater_than(
            xkis2, xkis1) * torch.greater_than(ykis2, ykis1)
...
class SIoULoss(GIoULoss):
    ...
    def __call__(self, pbox, gbox):
        ...
        ch = torch.maximum(cy, cyg) - torch.minimum(cy, cyg)
        ...
        thred = torch.pow(torch.to_tensor(2), 0.5) / 2
