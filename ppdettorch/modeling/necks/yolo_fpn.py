...
from ppdettorch.modeling.layers import DropBlock
...
def add_coord(x, data_format):
    ...
    gx = torch.Tensor(torch.arange(w) / ((w - 1.) * 2.0) - 1., x.dtype)
    gy = torch.Tensor(torch.arange(h) / ((h - 1.) * 2.0) - 1., x.dtype)
...
class SPP(nn.Module):
    def __init__(self,
        ...
        for size in pool_size:
            pool_block = nn.MaxPool2d(kernel_size=size,
                                      stride=1,
                                      padding=size // 2,
                                      ceil_mode=False)
            self.add_module('{}.pool1'.format(name), pool_block)
            self.pool.append(pool_block)
...
class PPYOLODetBlockCSP(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.data_format == 'NCHW':
            conv = torch.concat([conv_left, conv_right], dim=1)
        else:
            conv = torch.concat([conv_left, conv_right], dim=-1)
...
class YOLOv3FPN(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.num_blocks):
            ...
            if i > 0:
                in_channel += 512 // (2 ** i)
            yolo_block = YoloDetBlock(in_channel,
                                      channel=512 // (2 ** i),
                                      norm_type=norm_type,
                                      freeze_norm=freeze_norm,
                                      data_format=data_format,
            ...
            if i < self.num_blocks - 1:
                ...
                route = ConvBNLayer(ch_in=512 // (2 ** i),
                                    ch_out=256 // (2 ** i),
                                    filter_size=1,
                                    stride=1,
                                    padding=0,
...
class PPYOLOFPN(nn.Module):
    ...
    def __init__(self,
        ...
        for i, ch_in in enumerate(self.in_channels[::-1]):
            ...
            if i < self.num_blocks - 1:
                name = 'yolo_transition_{}'.format(i)
                route = ConvBNLayer(ch_in=channel,
                                    ch_out=256 // (2 ** i),
                                    filter_size=1,
                                    stride=1,
                                    padding=0,
...
class PPYOLOTinyFPN(nn.Module):
    ...
    def __init__(self,
        ...
        for i, (
            ...
            yolo_block = PPYOLOTinyDetBlock(ch_in,
                                            ch_out,
                                            name,
            ...
            if i < self.num_blocks - 1:
                name = 'yolo_transition_{}'.format(i)
                route = ConvBNLayer(ch_in=ch_out,
                                    ch_out=ch_out,
                                    filter_size=1,
                                    stride=1,
                                    padding=0,
...
class PPYOLOPAN(nn.Module):
    ...
    def __init__(self,
        ...
        for i, ch_in in enumerate(self.in_channels[::-1]):
            ...
            name = 'fpn_{}'.format(i)
            fpn_block = PPYOLODetBlockCSP(cfg, ch_in, channel, act, norm_type, name,
                                          data_format)
            ...
            if i < self.num_blocks - 1:
                ...
                route = ConvBNLayer(ch_in=channel * 2,
                                    ch_out=channel,
                                    filter_size=1,
                                    stride=1,
                                    padding=0,
        ...
        for i in reversed(range(self.num_blocks - 1)):
            ...
            route = ConvBNLayer(ch_in=fpn_channels[i + 1],
...
class YOLOv8CSPPAN(nn.Module):
    ...
    def forward(self, feats, for_mot=False):
        ...
        p_concat1 = torch.concat([down_feat1, f_out1], 1)
        pan_out1 = self.pan_n3(p_concat1)

        down_feat2 = self.down_conv1(pan_out1)
        p_concat2 = torch.concat([down_feat2, c5], 1)
