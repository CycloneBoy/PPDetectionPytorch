    ...
    def forward(self,
        ...
        if num_max_boxes == 0:
            assigned_labels = torch.full(
                [batch_size, num_anchors], bg_index, dtype=torch.int32)
        ...
        gt_labels_ind = torch.stack(
            ...
            dim=-1)
        bbox_cls_scores = torch.gather_nd(pred_scores, gt_labels_ind)
        ...
        max_metrics_per_instance = alignment_metrics.max(dim=-1, keepdim=True)
        max_ious_per_instance = (ious * mask_positive).max(dim=-1,
                                                           keepdim=True)
