...
class SimOTAAssigner(object):
    ...
    def get_in_gt_and_in_center_info(self, flatten_center_and_stride,
        ...
        is_in_cts = ct_deltas.min(dim=1) > 0
        is_in_cts_all = is_in_cts.sum(dim=1) > 0

        # in any of gts or gt centers, shape: [n_center]
        is_in_gts_or_centers_all = torch.logical_or(is_in_gts_all,
                                                    is_in_cts_all)

        is_in_gts_or_centers_all_inds = torch.nonzero(
            is_in_gts_or_centers_all).squeeze(1)

        # both in gts and gt centers, shape: [num_fg, num_gt]
        is_in_gts_and_centers = torch.logical_and(
            torch.gather(
    ...
    def dynamic_k_matching(self, cost_matrix, pairwise_ious, num_gt):
        ...
        dynamic_ks = torch.clip(topk_ious.sum(0).cast('int'), min=1)
        ...
        if extra_match_gts_mask.sum() > 0:
            ...
            cost_argmin = np.argmin(
                cost_matrix[extra_match_gts_mask, :], dim=1)
    ...
    def __call__(self,
        ...
        if self.use_vfl:
            ...
            losses_vfl = varifocal_loss(
                valid_pred_scores, vfl_score).reshape([num_valid_bboxes, num_gt])
            ...
            cost_matrix = (
                    ...
                    torch.logical_not(is_in_boxes_and_center).cast('float32') *
                    100000000)
        else:
            iou_cost = -torch.log(pairwise_ious + eps)
            gt_onehot_label = (F.one_hot(
                gt_labels.squeeze(-1).cast(torch.int64),
