...
def pad_gt(gt_labels, gt_bboxes, gt_scores=None):
    ...
    if isinstance(gt_labels, torch.Tensor) and isinstance(gt_bboxes,
        ...
        assert gt_labels.ndim == gt_bboxes.ndim and \
               gt_bboxes.ndim == 3
        pad_gt_mask = (
                gt_bboxes.sum(dim=-1, keepdim=True) > 0).astype(gt_bboxes.dtype)
...
def gather_topk_anchors(metrics, topk, largest=True, topk_mask=None, eps=1e-9):
    ...
    if topk_mask is None:
        topk_mask = (
                topk_metrics.max(dim=-1, keepdim=True) > eps).astype(metrics.dtype)
    is_in_topk = F.one_hot(topk_idxs, num_anchors).sum(
        dim=-2).astype(metrics.dtype)
...
def generate_anchors_for_grid_cell(feats,
    ...
    for feat, stride in zip(feats, fpn_strides):
        ...
        anchor = torch.stack(
            ...
            dim=-1).astype(dtype)
        anchor_point = torch.stack([shift_x, shift_y], dim=-1).astype(dtype)
        ...
        stride_tensor.append(
            torch.full(
                [num_anchors_list[-1], 1], stride).astype(dtype))
