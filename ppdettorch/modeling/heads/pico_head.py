...
from ..layers import ConvNormLayer
from ..necks.csp_pan import DPModule
...
class PicoHeadV2(GFLHead):
    ...
    def __init__(self,
        ...
        for i in range(len(fpn_stride)):
            head_cls = nn.Conv2d(in_channels=self.feat_in_chan,
            ...
            self.add_module("head_cls" + str(i), head_cls)
            self.head_cls_list.append(head_cls)
            head_reg = nn.Conv2d(in_channels=self.feat_in_chan,
                                 out_channels=4 * (self.reg_max + 1),
                                 kernel_size=1,
                                 stride=1,
                                 padding=0)
    ...
    def forward_train(self, fpn_feats):
        ...
        reg_list = torch.concat(reg_list, dim=1)
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        if num_total_pos > 0:
            pos_bbox_targets = torch.gather(
                flatten_bbox_targets, pos_inds, dim=0)
            pos_decode_bbox_pred = torch.gather(
                flatten_bboxes, pos_inds, dim=0)
            pos_reg = torch.gather(flatten_regs, pos_inds, dim=0)
            pos_strides = torch.gather(flatten_strides, pos_inds, dim=0)
            pos_centers = torch.gather(
                flatten_centers, pos_inds, dim=0) / pos_strides
