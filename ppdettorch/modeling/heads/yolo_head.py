    ...
    def forward(self, feats, targets=None):
        ...
        for i, feat in enumerate(feats):
            ...
            if self.data_format == 'NHWC':
                yolo_output = torch.permute(yolo_output, [0, 3, 1, 2])
        ...
        if self.training:
            ...
            if self.iou_aware:
                ...
                for i, out in enumerate(yolo_outputs):
                    ...
                    loc_t = x[:, :, :4, :]
                    cls_t = x[:, :, 5:, :]
                    y_t = torch.concat([loc_t, obj_t, cls_t], dim=2)
...
class YOLOXHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_reg = nn.ModuleList()  # reg [x,y,w,h] + obj
    ...
    def _init_weights(self):
        ...
        # bias_reg = torch.full([5], math.log(5.), dtype=self._dtype)
        # bias_reg[:2] = 0.
        # bias_reg[-1] = bias_cls
    ...
    def _generate_anchor_point(self, feat_sizes, strides, offset=0.):
        ...
        for feat_size, stride in zip(feat_sizes, strides):
            h, w = feat_size
            x = (torch.arange(w) + offset) * stride
            y = (torch.arange(h) + offset) * stride
            y, x = torch.meshgrid(y, x)
            anchor_points.append(torch.stack([x, y], dim=-1).reshape([-1, 2]))
    ...
    def forward(self, feats, targets=None):
        ...
        for i, feat in enumerate(feats):
            ...
            # reg prediction
            reg_xywh, obj_logit = torch.split(reg_pred, [4, 1], dim=1)
            reg_xywh = reg_xywh.flatten(2).permute([0, 2, 1])
            reg_pred_list.append(reg_xywh)
            # obj prediction
            obj_score = F.sigmoid(obj_logit)
            obj_score_list.append(obj_score.flatten(2).permute([0, 2, 1]))

        cls_score_list = torch.concat(cls_score_list, dim=1)
        reg_pred_list = torch.concat(reg_pred_list, dim=1)
        obj_score_list = torch.concat(obj_score_list, dim=1)

        # bbox decode
        anchor_points, stride_tensor, _ = \
            self._generate_anchor_point(feat_sizes, self.fpn_strides)

        run_device = cls_score_list.device
        anchor_points = anchor_points.to(run_device)
        stride_tensor = stride_tensor.to(run_device)

        reg_xy, reg_wh = torch.split(reg_pred_list, 2, dim=-1)
        reg_xy += (anchor_points / stride_tensor)
        reg_wh = torch.exp(reg_wh) * 0.5
        bbox_pred_list = torch.concat(
            [reg_xy - reg_wh, reg_xy + reg_wh], dim=-1)
    ...
    def get_loss(self, head_outs, targets):
        ...
        # label assignment
        center_and_strides = torch.concat(
            [anchor_points, stride_tensor, stride_tensor], dim=-1)
        ...
        labels = torch.Tensor(np.stack(label_list, dim=0))
        bbox_targets = torch.Tensor(np.stack(bbox_target_list, dim=0))
        ...
        if num_pos > 0:
            num_pos = torch.Tensor(num_pos, dtype=self._dtype).clip(min=1)
            loss_obj /= num_pos
            ...
            pred_cls_pos = torch.masked_select(
                pred_cls, cls_mask).reshape([-1, self.num_classes])
            assigned_cls_pos = torch.masked_select(labels, mask_positive)
