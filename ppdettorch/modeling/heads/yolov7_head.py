...
class YOLOv7Head(nn.Module):
    ...
    def __init__(self,
        ...
        self.anchors = torch.IntTensor(self.anchors)
        ...
        for i in range(self.num_levels):
            ...
            if self.use_aux:
                name_aux = 'yolo_output_aux_{}'.format(i)
                conv_aux = nn.Conv2d(
                ...
                yolo_output_aux = conv_aux
                self.add_module(name_aux, conv_aux)
                self.yolo_outputs_aux.append(yolo_output_aux)

            if self.use_implicit:
                ia = ImplicitA(self.in_channels[i])
                yolo_output_ia = ia
                self.add_module('yolo_output_ia_{}'.format(i), ia)
                self.ia.append(yolo_output_ia)

                im = ImplicitM(num_filters)
                yolo_output_im = im
                self.add_module('yolo_output_im_{}'.format(i), im)
    ...
    def fuse(self):
        if self.use_implicit:
            ...
            for i in range(len(self.yolo_outputs)):
                ...
                cc = torch.matmul(self.yolo_outputs[i].weight.reshape(
            ...
            for i in range(len(self.yolo_outputs)):
                ...
                self.yolo_outputs[i].weight.set_value(
                    self.yolo_outputs[i].weight * self.im[i].im.permute(1, 0, 2, 3))
    ...
    def make_grid(self, nx, ny, anchor):
        yv, xv = torch.meshgrid([
            torch.arange(
                ny, dtype=torch.int32), torch.arange(
                nx, dtype=torch.int32)
        ])
        grid = torch.stack((xv, yv), dim=2).reshape([1, 1, ny, nx, 2])
    ...
    def postprocessing_by_level(self, head_out, stride, anchor, ny, nx):
        ...
        run_device = head_out.device
        grid = grid.to(run_device)
        anchor_grid = anchor_grid.to(run_device)
        out = F.sigmoid(head_out)
        xy = (out[..., 0:2] * 2. - 0.5 + grid) * stride
        wh = (out[..., 2:4] * 2) ** 2 * anchor_grid
        lt_xy = (xy - wh / 2.)
        rb_xy = (xy + wh / 2.)
        bboxes = torch.concat((lt_xy, rb_xy), dim=-1)
    ...
    def post_process(self, head_outs, img_shape, scale_factor):
        ...
        for i, head_out in enumerate(head_outs):
            ...
            head_out = head_out.reshape(
                [bs, self.num_anchor, self.num_out_ch, ny, nx]).permute(0, 1, 3, 4, 2)
            ...
            score = score.reshape(
                [bs, self.num_anchor * ny * nx, self.num_classes]).permute(0, 2, 1)
            bbox_list.append(bbox)
            score_list.append(score)
        pred_bboxes = torch.concat(bbox_list, dim=1)
        pred_scores = torch.concat(score_list, dim=-1)
