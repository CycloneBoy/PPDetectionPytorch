...
from ppdettorch.modeling.bbox_utils import distance2bbox, bbox2distance, batch_distance2bbox, \
    bbox_overlaps
...
class DGQP(nn.Module):
    ...
    def __init__(self, reg_topk=4, reg_channels=64, add_mean=True):
        ...
        if add_mean:
            self.total_dim += 1
        self.reg_conv1 = self.add_module(
            'dgqp_reg_conv1',
            nn.Conv2d(
                in_channels=4 * self.total_dim,
                out_channels=self.reg_channels,
                kernel_size=1))
        self.reg_conv2 = self.add_module(
            'dgqp_reg_conv2',
            nn.Conv2d(
