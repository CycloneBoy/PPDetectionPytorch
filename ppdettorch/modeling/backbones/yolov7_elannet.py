...
class SPPCSPC(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        return self.cv7(torch.concat([y1, y2], dim=1))
...
class ImplicitM(nn.Module):
    def __init__(self, channel, mean=1., std=.02):
        ...
        self.im = nn.Parameter(torch.zeros([1, channel, 1, 1]))
        ...
        for i, (in_ch, out_ch) in enumerate(ch_settings[1:]):
            ...
            if i == 0:
                if self.arch in ['L', 'X', 'W6']:
                    ...
                    conv_layer = Conv(in_ch, _out_ch, 3, 2, bias=False, act=act)
                    ...
                    self.add_module('layers{}_stage{}_downc_layer'.format(layers_num, i + 1), downc_layer)
                ...
                if self.arch in ['L', 'X']:
                    ...
                    conv_res_layer = MPConvLayer(in_ch, in_ch, 0.5, depthwise, bias=False, act=act)
                    self.add_module('layers{}_stage{}_mpconv_layer'.format(layers_num, i + 1), conv_res_layer)
                    stage.append(conv_res_layer)
                    layers_num += 5  # 1 maxpool + 3 convs + 1 concat
                elif self.arch in ['tiny']:
                    # MP
                    mp_layer = MP(kernel_size=2, stride=2)
                    self.add_module('layers{}_stage{}_mp_layer'.format(layers_num, i + 1), mp_layer)
                    stage.append(mp_layer)
                    layers_num += 1
                elif self.arch in ['W6']:
                    # Conv
                    conv_layer = Conv(in_ch, out_ch, 3, 2, bias=False, act=act)
                    self.add_module('layers{}_stage{}_conv_layer'.format(layers_num, i + 1), conv_layer)
                    stage.append(conv_layer)
                    layers_num += 1
                elif self.arch in ['E6', 'D6', 'E6E']:
                    # DownC
                    downc_layer = DownC(in_ch, out_ch, 2, act=act)
                    self.add_module('layers{}_stage{}_downc_layer'.format(layers_num, i + 1), downc_layer)
                    stage.append(downc_layer)
                    layers_num += 1
                else:
            ...
            elan_layer = ELANBlock(elan_in_ch,
                                   mid_ch_settings[i][0],
                                   mid_ch_settings[i][1],
                                   out_ch,
                                   ...
                                   bias=False,
                                   act=act)
            self.add_module('layers{}_stage{}_elan_layer'.format(layers_num, i + 1), elan_layer)
            ...
            if i == len(ch_settings[1:]) - 1:
                if self.arch in ['L', 'X', 'W6', 'E6', 'D6', 'E6E']:
                    sppcspc_layer = SPPCSPC(out_ch, out_ch // 2, k=(5, 9, 13), act=act)
                    self.add_module('layers{}_stage{}_sppcspc_layer'.format(layers_num, i + 1), sppcspc_layer)
                    stage.append(sppcspc_layer)
                    layers_num += 1
                elif self.arch in ['tiny']:
                    sppelan_layer = SPPELAN(
                        out_ch, out_ch // 2, k=(5, 9, 13), act=act)
                    self.add_module('layers{}_stage{}_sppelan_layer'.format(layers_num, i + 1), sppelan_layer)
