...
class ChannelAttention(nn.Module):
    def __init__(self, channels=256):
        ...
        self.fc = nn.Conv2d(channels, channels, 1, 1, bias=True)
...
class CSPNeXt(nn.Module):
    ...
    def __init__(self,
        ...
        stem_list = [('conv1', BaseConv(3, stem_ch, 3, 2, act=act)),
                     ('conv2', BaseConv(stem_ch, stem_ch, 3, 1, act=act)),
                     ('conv3', BaseConv(stem_ch, stem_out_ch, 3, 1, act=act))]
        for key, val in stem_list:
            self.stem.add_module(key, val)
        ...
        for i, (in_ch, out_ch, n, shortcut, use_spp) in enumerate(arch_setting):
            ...
            conv_layer = Conv(in_channels, out_channels, 3, 2, act=act)
            ...
            csp_layer = CSPNeXtLayer(out_channels,
                                     out_channels,
                                     ...
                                     bias=False,
                                     act=act)
            self.add_module('layers{}_stage{}_cspnext_layer'.format(layers_num, i + 1), csp_layer)
