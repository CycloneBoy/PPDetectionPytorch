...
class ConvBNLayer(nn.Module):
    def __init__(self,
        ...
        self.conv = nn.Conv2d(
            ...
            groups=groups,
            bias=False)
        self.batch_norm = nn.BatchNorm2d(ch_out, track_running_stats=True)
        self.act = act
...
class Blocks(nn.Module):
    def __init__(self,
        ...
        for i in range(1, count):
            ...
            res_out = BasicBlock(ch_out,
                                 ch_out,
                                 norm_type=norm_type,
                                 norm_decay=norm_decay,
                                 freeze_norm=freeze_norm,
                                 data_format=data_format)
            self.add_module(block_name, res_out)
...
class DarkNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_stages - 1):
            ...
            downsample = DownSample(ch_in=int(ch_in[i]),
                                    ch_out=int(ch_in[i + 1]),
                                    norm_type=norm_type,
                                    norm_decay=norm_decay,
                                    freeze_norm=freeze_norm,
                                    data_format=data_format)
            self.add_module(down_name, downsample)
