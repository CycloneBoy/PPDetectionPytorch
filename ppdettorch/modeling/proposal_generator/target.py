...
def rpn_anchor_target(anchors,
    ...
    for i in range(batch_size):
        ...
        labels = torch.full(match_labels.shape, -1, dtype=torch.int32)
        if bg_inds.shape[0] > 0:
            labels = torch.scatter(labels, bg_inds, torch.zeros_like(bg_inds))
        if fg_inds.shape[0] > 0:
            labels = torch.scatter(labels, fg_inds, torch.ones_like(fg_inds))
        # Step3: make output  
        if gt_bbox.shape[0] == 0:
            matched_gt_boxes = torch.zeros([matches.shape[0], 4])
            tgt_delta = torch.zeros([matches.shape[0], 4])
        else:
            matched_gt_boxes = torch.gather(gt_bbox, matches)
...
def label_box(anchors,
    ...
    if n_gt == 0 or is_crowd is None:
        ...
        n_gt_crowd = torch.nonzero(is_crowd).shape[0]
    if iou.shape[0] == 0 or n_gt_crowd == n_gt:
        # No truth, assign everything to background
        default_matches = torch.full((iou.shape[1], ), 0, dtype='int64')
        default_match_labels = torch.full((iou.shape[1], ), 0, dtype=torch.int32)
    ...
    if n_gt_crowd > 0:
        ...
        if ignore_thresh > 0:
            crowd_iou = iou * mask
            valid = (torch.sum((crowd_iou > ignore_thresh).cast('int32'),
                                dim=0) > 0).cast('float32')
            iou = iou * (1 - valid) - valid

        # ignore the iou between anchor and crowded ground-truth
        iou = iou * (1 - mask) - mask

    matched_vals, matches = torch.topk(iou, k=1, dim=0)
    match_labels = torch.full(matches.shape, -1, dtype=torch.int32)
    # set ignored anchor with iou = -1
    neg_cond = torch.logical_and(matched_vals > -1,
                                  matched_vals < negative_overlap)
    match_labels = torch.where(neg_cond,
                                torch.zeros_like(match_labels), match_labels)
    match_labels = torch.where(matched_vals >= positive_overlap,
                                torch.ones_like(match_labels), match_labels)
    if allow_low_quality:
        highest_quality_foreach_gt = iou.max(dim=1, keepdim=True)
        pred_inds_with_highest_quality = torch.logical_and(
            iou > 0, iou == highest_quality_foreach_gt).cast('int32').sum(
                0, keepdim=True)
        match_labels = torch.where(pred_inds_with_highest_quality > 0,
                                    torch.ones_like(match_labels),
                                    match_labels)
...
def subsample_labels(labels,
    ...
    positive = torch.nonzero(
        torch.logical_and(labels != -1, labels != bg_label))
    ...
    if fg_num == 0 and bg_num == 0:
        fg_inds = torch.zeros([0], dtype=torch.int32)
        bg_inds = torch.zeros([0], dtype=torch.int32)
        return fg_inds, bg_inds

    # randomly select positive and negative examples

    negative = negative.cast('int32').flatten()
    bg_perm = torch.randperm(negative.numel(), dtype=torch.int32)
    bg_perm = torch.slice(bg_perm, axes=[0], starts=[0], ends=[bg_num])
    if use_random:
        bg_inds = torch.gather(negative, bg_perm)
    else:
        bg_inds = torch.slice(negative, axes=[0], starts=[0], ends=[bg_num])
    if fg_num == 0:
        fg_inds = torch.zeros([0], dtype=torch.int32)
        return fg_inds, bg_inds

    positive = positive.cast('int32').flatten()
    fg_perm = torch.randperm(positive.numel(), dtype=torch.int32)
    fg_perm = torch.slice(fg_perm, axes=[0], starts=[0], ends=[fg_num])
    if use_random:
        fg_inds = torch.gather(positive, fg_perm)
    else:
        fg_inds = torch.slice(positive, axes=[0], starts=[0], ends=[fg_num])
...
def generate_proposal_target(rpn_rois,
    ...
    for i, rpn_roi in enumerate(rpn_rois):
        ...
        # Step3: make output 
        rois_per_image = bbox if is_cascade else torch.gather(bbox,
                                                               sampled_inds)
        sampled_gt_ind = matches if is_cascade else torch.gather(matches,
                                                                  sampled_inds)
        if gt_bbox.shape[0] > 0:
            sampled_bbox = torch.gather(gt_bbox, sampled_gt_ind)
        else:
            num = rois_per_image.shape[0]
            sampled_bbox = torch.zeros([num, 4], dtype=torch.float32)
    ...
    new_rois_num = torch.concat(new_rois_num)
...
def sample_bbox(matches,
    ...
    if n_gt == 0:
        # No truth, assign everything to background
        gt_classes = torch.ones(matches.shape, dtype=torch.int32) * num_classes
        #return matches, match_labels + num_classes
    else:
        gt_classes = torch.gather(gt_classes, matches)
        gt_classes = torch.where(match_labels == 0,
                                  torch.ones_like(gt_classes) * num_classes,
                                  gt_classes)
        gt_classes = torch.where(match_labels == -1,
                                  torch.ones_like(gt_classes) * -1, gt_classes)
    if is_cascade:
        index = torch.arange(matches.shape[0])
    ...
    if fg_inds.shape[0] == 0 and bg_inds.shape[0] == 0:
        ...
        sampled_inds = torch.zeros([1], dtype=torch.int32)
    else:
        sampled_inds = torch.concat([fg_inds, bg_inds])
    sampled_gt_classes = torch.gather(gt_classes, sampled_inds)
...
def rasterize_polygons_within_box(poly, box, resolution):
    ...
    mask = torch.to_tensor(mask, dtype=torch.int32)
    return mask
...
def generate_mask_target(gt_segms, rois, labels_int32, sampled_gt_inds,
    ...
    for k in range(len(rois)):
        ...
        fg_inds = torch.nonzero(
            torch.logical_and(labels_per_im != -1, labels_per_im !=
                               num_classes))
        has_fg = True
        # generate fake roi if foreground is empty
        if fg_inds.numel() == 0:
            has_fg = False
            fg_inds = torch.ones([1, 1], dtype='int64')
        inds_per_im = sampled_gt_inds[k]
        inds_per_im = torch.gather(inds_per_im, fg_inds)

        rois_per_im = rois[k]
        fg_rois = torch.gather(rois_per_im, fg_inds)
        ...
        if len(gt_segms_per_im) > 0:
            ...
            results.append(torch.ones([resolution, resolution], dtype=torch.int32))

        fg_classes = torch.gather(labels_per_im, fg_inds)
        weight = torch.ones([fg_rois.shape[0]], dtype=torch.float32)
        if not has_fg:
            # now all sampled classes are background
            # which will cause error in loss calculation,
            # make fake classes with weight of 0.
            fg_classes = torch.zeros([1], dtype=torch.int32)
            weight = weight - 1
        tgt_mask = torch.stack(results)
        tgt_mask.stop_gradient = True
        fg_rois.stop_gradient = True

        mask_index.append(fg_inds)
        mask_rois.append(fg_rois)
        mask_rois_num.append(torch.shape(fg_rois)[0])
        tgt_classes.append(fg_classes)
        tgt_masks.append(tgt_mask)
        tgt_weights.append(weight)

    mask_index = torch.concat(mask_index)
    mask_rois_num = torch.concat(mask_rois_num)
    tgt_classes = torch.concat(tgt_classes, dim=0)
    tgt_masks = torch.concat(tgt_masks, dim=0)
    tgt_weights = torch.concat(tgt_weights, dim=0)
...
def libra_sample_pos(max_overlaps, max_classes, pos_inds, num_expected):
    if len(pos_inds) <= num_expected:
        ...
        return torch.to_tensor(sampled_inds)
...
def libra_label_box(anchors, gt_boxes, gt_classes, positive_overlap,
    ...
    if len(gt_boxes) > 0:
        ...
        overlaps_argmax = proposal_to_gt_overlaps.argmax(dim=1)
        overlaps_max = proposal_to_gt_overlaps.max(dim=1)
    ...
    gt_overlaps = torch.to_tensor(gt_overlaps)
    matches = torch.to_tensor(matches)

    matched_vals = torch.max(gt_overlaps, dim=1)
    match_labels = torch.full(matches.shape, -1, dtype=torch.int32)
    match_labels = torch.where(matched_vals < negative_overlap,
                                torch.zeros_like(match_labels), match_labels)
    match_labels = torch.where(matched_vals >= positive_overlap,
                                torch.ones_like(match_labels), match_labels)
...
def libra_sample_bbox(matches,
    ...
    if is_cascade_rcnn:
        fg_inds = torch.nonzero(matched_vals >= fg_thresh)
        bg_inds = torch.nonzero(matched_vals < bg_thresh)
    else:
        matched_vals_np = matched_vals.numpy()
        match_labels_np = match_labels.numpy()

        # sample fg
        fg_inds = torch.nonzero(matched_vals >= fg_thresh).flatten()
        ...
        # sample bg
        bg_inds = torch.nonzero(matched_vals < bg_thresh).flatten()
...
def libra_generate_proposal_target(rpn_rois,
    ...
    for i, rpn_roi in enumerate(rpn_rois):
        ...
        # Step3: make output
        rois_per_image = torch.gather(bbox, sampled_inds)
        sampled_gt_ind = torch.gather(matches, sampled_inds)
        sampled_bbox = torch.gather(gt_bbox, sampled_gt_ind)
        sampled_overlap = torch.gather(matched_vals, sampled_inds)
