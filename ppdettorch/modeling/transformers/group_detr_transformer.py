    ...
    def get_reference_points(spatial_shapes, valid_ratios, offset=0.5):
        ...
        for i, (H, W) in enumerate(spatial_shapes):
            ref_y, ref_x = torch.meshgrid(
                torch.arange(end=H) + offset, torch.arange(end=W) + offset)
...
class DINOTransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        if self.dual_queries:
            ...
            bs, num_queries, n_model = torch.shape(q)
            q = torch.concat(q.split(dual_groups + 1, axis=1), axis=0)
            k = torch.concat(k.split(dual_groups + 1, axis=1), axis=0)
            tgt = torch.concat(tgt.split(dual_groups + 1, axis=1), axis=0)
        ...
        # trace back
        if self.dual_queries:
            tgt = torch.concat(tgt.split(dual_groups + 1, axis=0), axis=1)
...
class DINOTransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(inter_ref_bboxes)

        return output, reference_points


@register
class GroupDINOTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.dual_groups = dual_groups
        if self.dual_queries:
            self.denoising_class_embed_groups = nn.ModuleList([
                nn.Embedding(
                    num_classes,
                    hidden_dim,
                    weight_attr=ParamAttr(initializer=nn.initializer.Normal()))
        ...
        if learnt_init_query:
            ...
            if self.dual_queries:
                self.tgt_embed_dual = nn.ModuleList([
        ...
        if self.dual_queries:
            self.enc_bbox_head_dq = nn.ModuleList([
                MLP(hidden_dim, hidden_dim, 4, num_layers=3)
                for i in range(self.dual_groups)
            ])
            self.enc_score_head_dq = nn.ModuleList([
    ...
    def _get_encoder_input(self, feats, pad_mask=None):
        ...
        for i, feat in enumerate(proj_feats):
            bs, _, h, w = torch.shape(feat)
            spatial_shapes.append(torch.concat([h, w]))
            # [b,c,h,w] -> [b,h*w,c]
        ...
        # [b, l]
        mask_flatten = None if pad_mask is None else torch.concat(mask_flatten,
                                                                   1)
        # [b, l, c]
        lvl_pos_embed_flatten = torch.concat(lvl_pos_embed_flatten, 1)
        # [num_levels, 2]
        spatial_shapes = torch.tensor(
            torch.stack(spatial_shapes).astype('int64'))
        # [l] start index of each level
        level_start_index = torch.concat([
            torch.zeros(
    ...
    def _get_encoder_output_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            if memory_mask is not None:
                ...
                valid_H = torch.sum(mask_[:, :, 0], 1)
                valid_W = torch.sum(mask_[:, 0, :], 1)
            else:
                valid_H, valid_W = h, w
            ...
            valid_WH = torch.stack([valid_W, valid_H], -1).reshape(
        ...
        output_anchors = torch.concat(output_anchors, 1)
        valid_mask = ((output_anchors > self.eps) *
                      (output_anchors < 1 - self.eps)).all(-1, keepdim=True)
        output_anchors = torch.log(output_anchors / (1 - output_anchors))
        if memory_mask is not None:
            valid_mask = (valid_mask * (memory_mask.unsqueeze(-1) > 0)) > 0
        output_anchors = torch.where(valid_mask, output_anchors,
    ...
    def _get_decoder_input(self,
        ...
        if self.dual_queries:
            ...
            for g_id in range(self.dual_groups):
                ...
                _, topk_ind_gid = torch.topk(
                    enc_outputs_class_gid.max(-1), self.num_queries, axis=1)
                # extract region proposal boxes
                batch_ind = torch.arange(end=bs, dtype=topk_ind_gid.dtype)
                batch_ind = batch_ind.unsqueeze(-1).tile([1, self.num_queries])
                topk_ind_gid = torch.stack([batch_ind, topk_ind_gid], axis=-1)
                topk_coords_unact_gid = torch.gather_nd(
                ...
                enc_topk_logits_gid = torch.gather_nd(enc_outputs_class_gid,
                                                       topk_ind_gid)
            ...
            enc_topk_bboxes = torch.concat(
                [enc_topk_bboxes] + enc_topk_bboxes_groups, 1)
            enc_topk_logits = torch.concat(
                [enc_topk_logits] + enc_topk_logits_groups, 1)
            reference_points = torch.concat(
                [reference_points] + reference_points_groups, 1)
            topk_ind = torch.concat([topk_ind] + topk_ind_groups, 1)
        ...
        if self.learnt_init_query:
            ...
            if self.dual_queries:
                target = torch.gather_nd(output_memory[0], topk_ind)
                target_groups = []
                for g_id in range(self.dual_groups):
                    target_gid = torch.gather_nd(output_memory[g_id + 1],
                                                  topk_ind_groups[g_id])
                    target_groups.append(target_gid)
                target = torch.concat([target] + target_groups, 1).detach()
            else:
                target = torch.gather_nd(output_memory, topk_ind).detach()
        ...
        if denoising_bbox is not None:
            if isinstance(denoising_bbox, list) and isinstance(
                ...
                if denoising_bbox[0] is not None:
                    reference_points_list = torch.split(
