...
class MaskDINO(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        target = torch.gather_nd(output_memory, topk_ind)
        reference_points_unact = torch.gather_nd(enc_bboxes_unact,
    ...
    def _get_pred_class_and_mask(self, query_embed, mask_feat):
        ...
        _, _, h, w = torch.shape(mask_feat)
        # [b, q, c] x [b, c, h, w] -> [b, q, h, w]
        out_mask = torch.bmm(mask_query_embed, mask_feat.flatten(2)).reshape(
            [0, 0, h, w])
