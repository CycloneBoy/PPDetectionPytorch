...
class DINOTransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.norm1 = nn.LayerNorm(
            d_model, weight_attr=weight_attr, bias=bias_attr)
...
class DINOTransformerDecoder(nn.Module):
    def __init__(self,
        ...
        self.norm = nn.LayerNorm(
            hidden_dim, weight_attr=weight_attr, bias=bias_attr)
    ...
    def forward(self,
        ...
        return torch.stack(intermediate), torch.stack(inter_bboxes)
...
class DINOTransformer(nn.Module):
    ...
    def _get_encoder_output_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            grid_y, grid_x = torch.meshgrid(
                torch.arange(end=h), torch.arange(end=w))
            grid_xy = torch.stack([grid_x, grid_y], -1).astype(memory.dtype)
    ...
    def _get_decoder_input(self,
        ...
        batch_ind = torch.arange(end=bs).astype(topk_ind.dtype)
        batch_ind = batch_ind.unsqueeze(-1).tile([1, self.num_queries])
        topk_ind = torch.stack([batch_ind, topk_ind], axis=-1)
        reference_points_unact = torch.gather_nd(enc_outputs_coord_unact,
