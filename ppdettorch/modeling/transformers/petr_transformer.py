...
class MSDeformableAttention(nn.Module):
    def __init__(self,
        ...
        self.sampling_offsets = nn.Linear(
            ...
            bias=ParamAttr(learning_rate=lr_mult))
...
@register
class MultiScaleDeformablePoseAttention(nn.Module):
    ...
    def forward(self,
        ...
        if reference_points.shape[-1] == self.num_points * 2:
            ...
            w = torch.clip(x2 - x1, min=1e-4)
            h = torch.clip(y2 - y1, min=1e-4)
            wh = torch.concat([w, h], axis=-1)[:, :, None, :, None, :]
...
@register
class PETR_TransformerDecoderLayer(nn.Module):
...
@register
class PETR_TransformerDecoder(nn.Module):
...
@register
class PETR_DeformableTransformerDecoder(nn.Module):
    __inject__ = ['decoder_layer']
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                reference_points_input = reference_points[:, :, None] * \
                    torch.concat([valid_ratios, valid_ratios], -1)[:, None]
            else:
                assert reference_points.shape[-1] == 2
                reference_points_input = reference_points[:, :, None] * \
                    valid_ratios[:, None]
...
class PETRTransformer(nn.Module):
    ...
    def init_layers(self):
        ...
        #torch.create_parameter
        self.level_embeds = torch.create_parameter(
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask,
        ...
        for lvl, (H, W) in enumerate(spatial_shapes):
            ...
            grid = torch.concat([grid_x.unsqueeze(-1), grid_y.unsqueeze(-1)],
                                 -1)
            ...
            _cur += (H * W)
        output_proposals = torch.concat(proposals, 1)
    ...
    def get_reference_points(spatial_shapes, valid_ratios):
        ...
        reference_points = torch.concat(reference_points_list, 1)
    ...
    def get_valid_ratio(self, mask):
        ...
        _, H, W = mask.shape
        valid_H = torch.sum(mask[:, :, 0].astype('float'), 1)
        valid_W = torch.sum(mask[:, 0, :].astype('float'), 1)
    ...
    def get_proposal_pos_embed(self,
        ...
        scale = 2 * math.pi
        dim_t = torch.arange(num_pos_feats, dtype="float32")
    ...
    def forward(self,
        ...
        if self.training:
            ...
            hm_mask = torch.slice(
                ...
                ends=level_start_index[1],
                axes=[1])
            hm_reference_points = torch.slice(
                reference_points,
                starts=level_start_index[0],
                ends=level_start_index[1],
                axes=[1])[:, :, :1, :]
        ...
        if self.as_two_stage:
            ...
            #torch.take_along_axis 对应torch.gather
            topk_kpts_unact = torch.take_along_axis(enc_outputs_kpt_unact,
                                                     topk_proposals, 1)
    ...
    def forward_refine(self,
        ...
        query_pos, query = torch.split(refine_query_embedding, 2, axis=1)
